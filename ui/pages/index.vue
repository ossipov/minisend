<template>
  <div class="container max-w-5xl mx-auto p-5">
    ...
    <div
      v-if="server_error && !loading"
      class="p-10 border-2 rounded border-red-700 text-2xl text-center text-red-800"
    >
      Sorry, something's wrong with API server 😥
    </div>

    <div
      class="flex px-2 text-gray-700 py-1 rounded mb-6 text-sm w-full"
      v-if="mails.total > 10"
    >
      <span class="flex-grow">
        <span class="text-gray-900">Page:</span> {{ mails.current_page }} /
        {{ mails.last_page }}
      </span>
    ...
    <div
      v-if="mails.total === 0 && !server_error && !loading"
      class="p-10 border-2 rounded border-gray-700 text-2xl text-center text-gray-800"
    >
      No Mail found 😋
    </div>
    ...
    <table class="mail-list w-full" v-if="mails.data && !loading">
      <tbody>
        ...
        >
          <td
            class="text-center block sm:table-cell py-3 px-4 w-full sm:w-1/12"
          >
